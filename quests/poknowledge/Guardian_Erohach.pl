...
sub EVENT_SAY {
	if($text=~/Hail/i) {
		quest::say("Well met! Do you seek a replica of the [Power of Undead's Bane], gear for a starting [paladin], or some other kind of [help]?"); #Class we offer
	}
	if($text=~/Power of Undead's Bane/i) {
		quest::say("If you seek that power then bring me five thousand platinum and the following items: a familiar ring, an essence of sunlight, some hearty shissar blood, and a forged deathsteel long sword."); #Bot Paladin Epic Needs
	}
	if($text=~/paladin/i) {
		quest::say("Merely hand over that Gloomingdeep Lantern and I shall give you a ghoulbane and a mithril breastplate."); #Starter Gear - Lantern Hand-In
	}
	if($text=~/help/i) {
...
my $cash = $platinum;
	if ($cash >= 5000) { #5000 Platinum, Exact Change Only
		if (plugin::check_handin(\%itemcount, 60328 => 1, 22527 => 1, 96909 => 1, 11887 => 1)) {
			...
			quest::shout2("Let it be known that on this day, $name has braved the challenges and has been rewarded the Power of Undead's Bane!");
			quest::ding();
			quest::summonitem(147506); #Copied version of the epic -- temporary and level 45 requirement to prevent twinks.
		}
	}
	elsif (plugin::check_handin(\%itemcount, 9979 => 1)) {
		quest::say("May they bring you fortune and glory!");
		quest::ding();
		quest::summonitem(5403); #Ghoulbane
