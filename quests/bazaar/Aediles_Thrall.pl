...
#
#
# Updated by Surumon on 9/23/2018 - Removed Slavery References, Adjusted Costs and Levels
#
############################################
...
my $fourthbotcost  = 800;
my $fifthbotlevel  = 40;
my $fifthbotcost   = 1200;
   ...
   if(defined $qglobals{bot_spawn_limit} && (quest::spawnbotcount() > $qglobals{bot_spawn_limit})) {
      if(($ulevel >= $firstbotlevel) && ($qglobals{bot_spawn_limit} <= 0)) {
         ...
         if($text=~/Help/i) {
            ...
            quest::say("Well, I kin offer ye a [product] ye can't find no place else.");
         }
         if($text=~/Product/i) {
         ...
         if($text=~/AID/i) {
            quest::emote("looks around the room again");
            quest::say("It be 'Assisting Individuals Delivery' and it means I kin supply ye with [Individuals].");
         }
